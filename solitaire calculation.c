...
void update_table(void){
    ...
    for(i = 0; i < 4; i++) {
        ...
        while (stuck[i][elements_count].suit != NULL && elements_count < NUM_STOCK) { // 各行の要素数を計算する
            elements_count++;
        }
        if (max_elements < elements_count) { // 各行の要素数が最大値よりも大きい場合、更新する
